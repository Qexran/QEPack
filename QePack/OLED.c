...
/**
  ...
  */
void OLED_GPIO_Init(emOledDevNumTdf emDevNum)
{
//	uint32_t i, j;
...
// 	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	/*释放SCL和SDA*/
	OLED_W_SCL(emDevNum, 1);
	OLED_W_SDA(emDevNum, 1);
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_Start(emOledDevNumTdf emDevNum)
{
	OLED_W_SDA(emDevNum, 1);		//释放SDA，确保SDA为高电平
	OLED_W_SCL(emDevNum, 1);		//释放SCL，确保SCL为高电平
	OLED_W_SDA(emDevNum, 0);		//在SCL高电平期间，拉低SDA，产生起始信号
	OLED_W_SCL(emDevNum, 0);		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_Stop(emOledDevNumTdf emDevNum)
{
	OLED_W_SDA(emDevNum, 0);		//拉低SDA，确保SDA为低电平
	OLED_W_SCL(emDevNum, 1);		//释放SCL，使SCL呈现高电平
	OLED_W_SDA(emDevNum, 1);		//在SCL高电平期间，释放SDA，产生终止信号
}
...
/**
  ...
  * 返 回 值：无
  */
void OLED_I2C_SendByte(emOledDevNumTdf emDevNum, uint8_t Byte)
{
	uint8_t i;
	...
	for (i = 0; i < 8; i++)
	{
		...
		/*两个!的作用是，让所有非零的值变为1*/
		OLED_W_SDA(emDevNum, !!(Byte & (0x80 >> i))); // 1000 0000 
		OLED_W_SCL(emDevNum, 1);	//释放SCL，从机在SCL高电平期间读取SDA
		OLED_W_SCL(emDevNum, 0);	//拉低SCL，主机开始发送下一位数据
	}
	
	OLED_W_SCL(emDevNum, 1);		//额外的一个时钟，不处理应答信号
	OLED_W_SCL(emDevNum, 0);
...
void vOledDeviceInit(stOledStaticParamTdf *pstInit, emOledDevNumTdf emDevNum)
{
	...
	OLED_WriteCommand(emDevNum, 0xD5);					//设置显示时钟分频比/振荡器频率
	OLED_WriteCommand(emDevNum, 0x80);					//0x00~0xFF
					
	OLED_WriteCommand(emDevNum, 0xA8);					//设置多路复用率
	OLED_WriteCommand(emDevNum, 0x3F);					//0x0E~0x3F
					
	OLED_WriteCommand(emDevNum, 0xD3);					//设置显示偏移
	OLED_WriteCommand(emDevNum, 0x00);					//0x00~0x7F
					
	OLED_WriteCommand(emDevNum, 0x40);					//设置显示开始行，0x40~0x7F
					
	OLED_WriteCommand(emDevNum, 0xA1);					//设置左右方向，0xA1正常，0xA0左右反置
					
	OLED_WriteCommand(emDevNum, 0xC8);					//设置上下方向，0xC8正常，0xC0上下反置
				
	OLED_WriteCommand(emDevNum, 0xDA);					//设置COM引脚硬件配置
	OLED_WriteCommand(emDevNum, 0x12);				
					
	OLED_WriteCommand(emDevNum, 0x81);					//设置对比度
	OLED_WriteCommand(emDevNum, 0xCF);					//0x00~0xFF
				
	OLED_WriteCommand(emDevNum, 0xD9);					//设置预充电周期
	OLED_WriteCommand(emDevNum, 0xF1);				
				
	OLED_WriteCommand(emDevNum, 0xDB);					//设置VCOMH取消选择级别
	OLED_WriteCommand(emDevNum, 0x30);				
				
	OLED_WriteCommand(emDevNum, 0xA4);					//设置整个显示打开/关闭
				
	OLED_WriteCommand(emDevNum, 0xA6);					//设置正常/反色显示，0xA6正常，0xA7反色
				
	OLED_WriteCommand(emDevNum, 0x8D);					//设置充电泵
	OLED_WriteCommand(emDevNum, 0x14);				
				
	OLED_WriteCommand(emDevNum, 0xAF);					//开启显示
					
	vOledClear(emDevNum);								//清空显存数组
	vOledUpdate(emDevNum);								//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED_SetCursor(emOledDevNumTdf emDevNum, uint8_t Page, uint8_t X)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED_WriteCommand(emDevNum, 0xB0 | Page);					//设置页位置
	OLED_WriteCommand(emDevNum, 0x10 | ((X & 0xF0) >> 4));	//设置X位置高4位
	OLED_WriteCommand(emDevNum, 0x00 | (X & 0x0F));			//设置X位置低4位
}
...
{
	static const uint32_t auPow10[] = {1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000, 1000000000};
	static const uint32_t auPow2[] = {1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768};
	static const uint32_t auPow16[] = {1, 16, 256, 4096, 65536, 1048576, 16777216, 268435456};
	
	if (X == 10 && Y < sizeof(auPow10) / sizeof(auPow10[0]))
	{
		return auPow10[Y];
	}
	else if (X == 2 && Y < sizeof(auPow2) / sizeof(auPow2[0]))
	{
		return auPow2[Y];
	}
	else if (X == 16 && Y < sizeof(auPow16) / sizeof(auPow16[0]))
	{
		return auPow16[Y];
	}
...
void vOledUpdate(emOledDevNumTdf emDevNum)
{
	...
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(emDevNum, j, 0);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData(
			emDevNum, 
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j], 
			128
		);
	}
}
...
void vOledUpdateArea(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	...
	{
		/*设置光标位置为相关页的指定列*/
		OLED_SetCursor(emDevNum, j, X);
		/*连续写入Width个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData(
			emDevNum, 
			&astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j][X], 
			Width
		);
	}
}
...
void vOledClear(emOledDevNumTdf emDevNum)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		memset(astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j], 0x00, 128);	//批量清零
	}
}
...
void vOledClearArea(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		...
		{
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j / 8][i] &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
		}
	}
}
...
void vOledReverse(emOledDevNumTdf emDevNum)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j][i] ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void vOledReverseArea(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[j / 8][i] ^= 0x01 << (j % 8);	//将显存数组指定数据取反
		}
	}
}
...
void vOledShowChar(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, char Char)
{
	if (FontSize == OLED_8X16)		//字体为宽8像素，高16像素
	{
		...
		vOledShowImage(emDevNum, X, Y, 8, 16, OLED_F8x16[Char - ' ']);
	}
	else if(FontSize == OLED_6X8)	//字体为宽6像素，高8像素
	{
		/*将ASCII字模库OLED_F6x8的指定数据以6*8的图像格式显示*/
		vOledShowImage(emDevNum, X, Y, 6, 8, OLED_F6x8[Char - ' ']);
	}
}
...
void vOledShowString(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, char *String)
{
	...
	for (i = 0; String[i] != '\0'; i++)		//遍历字符串的每个字符
	{
		...
		vOledShowChar(emDevNum, X + i * FontSize, Y, FontSize, String[i]);
	}
}
...
void vOledShowNum(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位							
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		vOledShowChar(emDevNum, X + i * FontSize, Y, FontSize, Number / OLED_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void vOledShowSignedNum(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, int32_t Number, uint8_t Length)
{
	...
	if (Number >= 0)						//数字大于等于0
	{
		vOledShowChar(emDevNum, X, Y, FontSize, '+');	//显示+号
	...
	for (i = 0; i < Length; i++)			//遍历数字的每一位								
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		vOledShowChar(emDevNum, X + (i + 1) * FontSize, Y, FontSize, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void vOledShowHexNum(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位
	{
		...
		if (SingleNumber < 10)			//单个数字小于10
		{
			...
			/*+ '0' 可将数字转换为字符格式*/
			vOledShowChar(emDevNum, X + i * FontSize, Y, FontSize, SingleNumber + '0');
		}
		else							//单个数字大于10
		{
			/*调用OLED_ShowChar函数，显示此数字*/
			/*+ 'A' 可将数字转换为从A开始的十六进制字符*/
			vOledShowChar(emDevNum, X + i * FontSize, Y, FontSize, SingleNumber - 10 + 'A');
		}
	}
}
...
void vOledShowBinNum(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)		//遍历数字的每一位	
	{
		...
		/*+ '0' 可将数字转换为字符格式*/
		vOledShowChar(emDevNum, X + i * FontSize, Y, FontSize, Number / OLED_Pow(2, Length - i - 1) % 2 + '0');
	}
}
...
void vOledShowFloatNum(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, emOledFontSizeTdf FontSize, double Number, uint8_t IntLength, uint8_t FraLength)
{
	...
	/*显示整数部分*/
	vOledShowNum(emDevNum, X + FontSize, Y, FontSize, IntNum, IntLength);
	
	/*显示小数点*/
	vOledShowChar(emDevNum, X + (IntLength + 1) * FontSize, Y, FontSize, '.');
	
	/*显示小数部分*/
	vOledShowNum(emDevNum, X + (IntLength + 2) * FontSize, Y, FontSize, FraNum, FraLength);
}
...
void vOledShowChinese(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, char *Chinese)
{
	...
	for (i = 0; Chinese[i] != '\0'; i ++)		//遍历汉字串
	{
		...
		if (pChinese >= OLED_CHN_CHAR_WIDTH)
		{
			...
			vOledShowImage(emDevNum, X + ((i + 1) / OLED_CHN_CHAR_WIDTH - 1) * 16, Y, 16, 16, OLED_CF16x16[pIndex].Data);
		}
	}
}
...
void vOledShowImage(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	/*将图像所在区域清空*/
	vOledClearArea(emDevNum, X, Y, Width, Height);
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < Width; i ++)
		{
			...
			/*显示图像在当前页的内容*/
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[Y / 8 + j][X + i] |= Image[j * Width + i] << (Y % 8);
			...
			/*显示图像在下一页的内容*/
			astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[Y / 8 + j + 1][X + i] |= Image[j * Width + i] >> (8 - Y % 8);
		}
	}
}
...
void vOledDrawPoint(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y)
{
	...
	/*将显存数组指定位置的一个Bit数据置1*/
	astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[Y / 8][X] |= 0x01 << (Y % 8);
}
...
uint8_t uOledGetPoint(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y)
{
	...
	/*判断指定位置的数据*/
	if (astOledDeviceParam[emDevNum].stRunningParam.aucDisplayBuf[Y / 8][X] & 0x01 << (Y % 8))
	{
		return 1;	//为1，返回1
	}
...
void vOledDrawLine(emOledDevNumTdf emDevNum, uint8_t X0, uint8_t Y0, uint8_t X1, uint8_t Y1)
{
	...
	if (y0 == y1)		//横线单独处理
	{
		...
		for (x = x0; x <= x1; x ++)
		{
			vOledDrawPoint(emDevNum, x, y0);	//依次画点
		}
	}
	else if (x0 == x1)	//竖线单独处理
	{
		...
		for (y = y0; y <= y1; y ++)
		{
			vOledDrawPoint(emDevNum, X0, y);	//依次画点
		}
	}
	else				//斜线
	{
		...
		/*画起始点，同时判断标志位，将坐标换回来*/
		if (yflag && xyflag){vOledDrawPoint(emDevNum, y, -x);}
		else if (yflag)		{vOledDrawPoint(emDevNum, x, -y);}
		else if (xyflag)	{vOledDrawPoint(emDevNum, y, x);}
		else				{vOledDrawPoint(emDevNum, x, y);}
...
void vOledDrawRectangle(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Width, uint8_t Height, emOledFillStateTdf IsFilled)
{
	...
	if (!IsFilled)		//指定矩形不填充
	{
		...
		for (i = X; i < X + Width; i ++)
		{
			vOledDrawPoint(emDevNum, i, Y);
			vOledDrawPoint(emDevNum, i, Y + Height - 1);
		}
		/*遍历左右Y坐标，画矩形左右两条线*/
		for (i = Y; i < Y + Height; i ++)
		{
			vOledDrawPoint(emDevNum, X, i);
			vOledDrawPoint(emDevNum, X + Width - 1, i);
		}
	}
	else				//指定矩形填充
	{
...
void vOledDrawTriangle(emOledDevNumTdf emDevNum, uint8_t X0, uint8_t Y0, uint8_t X1, uint8_t Y1, uint8_t X2, uint8_t Y2, emOledFillStateTdf IsFilled)
{
	...
	{
		/*调用画线函数，将三个点用直线连接*/
		vOledDrawLine(emDevNum, X0, Y0, X1, Y1);
		vOledDrawLine(emDevNum, X0, Y0, X2, Y2);
		vOledDrawLine(emDevNum, X1, Y1, X2, Y2);
	}
	else					//指定三角形填充
	{
		/*找到三个点最小的X、Y坐标*/
		...
		for (i = minx; i <= maxx; i ++)
		{
			...
			for (j = miny; j <= maxy; j ++)
			{
				...
				/*如果在，则画点，如果不在，则不做处理*/
				if (OLED_pnpoly(3, vx, vy, i, j)) {vOledDrawPoint(emDevNum, i, j);}
			}
		}
	}
}
...
void vOledDrawCircle(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Radius, emOledFillStateTdf IsFilled)
{
	...
	/*画每个八分之一圆弧的起始点*/
	vOledDrawPoint(emDevNum, X + x, Y + y);
	vOledDrawPoint(emDevNum, X - x, Y - y);
	vOledDrawPoint(emDevNum, X + y, Y + x);
	vOledDrawPoint(emDevNum, X - y, Y - x);
	...
	while (x < y)		//遍历X轴的每个点
	{
		...
		if (IsFilled)	//指定圆填充
		{
			...
			{
				/*在指定区域画点，填充部分圆*/
				vOledDrawPoint(emDevNum, X + x, Y + j);
				vOledDrawPoint(emDevNum, X - x, Y + j);
			}
			...
			{
				/*在指定区域画点，填充部分圆*/
				vOledDrawPoint(emDevNum, X - y, Y + j);
				vOledDrawPoint(emDevNum, X + y, Y + j);
			}
		}
	}
...
void vOledDrawArc(emOledDevNumTdf emDevNum, uint8_t X, uint8_t Y, uint8_t Radius, int16_t StartAngle, int16_t EndAngle, emOledFillStateTdf IsFilled)
{
	...
	if (OLED_IsInAngle(x, y, StartAngle, EndAngle))	{vOledDrawPoint(emDevNum, X + x, Y + y);}
	if (OLED_IsInAngle(-x, -y, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X - x, Y - y);}
	if (OLED_IsInAngle(y, x, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X + y, Y + x);}
	if (OLED_IsInAngle(-y, -x, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X - y, Y - x);}
	...
	if (IsFilled)	//指定圆弧填充
	{
		...
		for (j = -y; j < y; j ++)
		{
			...
			if (OLED_IsInAngle(0, j, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X, Y + j);}
		}
	}
	...
	while (x < y)		//遍历X轴的每个点
	{
		...
		if (IsFilled)	//指定圆弧填充
		{
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(x, j, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X + x, Y + j);}
				if (OLED_IsInAngle(-x, j, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X - x, Y + j);}
			}
			...
			{
				/*在填充圆的每个点时，判断指定点是否在指定角度内，在，则画点，不在，则不做处理*/
				if (OLED_IsInAngle(-y, j, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X - y, Y + j);}
				if (OLED_IsInAngle(y, j, StartAngle, EndAngle)) {vOledDrawPoint(emDevNum, X + y, Y + j);}
			}
		}
	}
