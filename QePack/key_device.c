/**
  ...
  * @version    V1.0.5
  * @date       2026/1/29
  * @brief      按键驱动，基于 STM32 HAL 库（修复单击失效+参数类型错误）
  *
  */
...
/// @brief      按键周期执行（处理消抖、单击、双击、长按逻辑，支持功能禁用）
/// @param      emDevNum   ：设备号
...
void vKeyDevicePeriodExecute(emKeyDevNumTdf emDevNum)
{
    ...
    {
        // 2.1 按键按下状态 - 先累加长按计数（无论长按功能是否启用，确保单击能检测到按下）
...
/// @brief      清除按键事件
/// @param      emDevNum   ：设备号
void vKeyClearEvent(emKeyDevNumTdf emDevNum) // 修复参数类型错误
{
