 ...
 * @param       timeout: 等待数据帧最大超时时间，单位：毫秒（ms）
 ...
 * @param       dat    : 读取到的数据
 * @param       timeout: 等待数据的最大超时时间，单位：毫秒（ms）
 * @retval      0: 读取失败
 *              其他值: 读取到数据的长度
 */
...
uint8_t atk_ms901m_init(emUartDevNumTdf emDevNum)
{
    ...
    /* 获取ATK-MS901M陀螺仪满量程 */
    ret = atk_ms901m_read_reg_by_id(emDevNum, ATK_MS901M_FRAME_ID_REG_GYROFSR, &g_atk_ms901m_fsr.gyro, 200);
    if (ret == 0)
    {
    ...
    /* 获取ATK-MS901M加速度计满量程 */
    ret = atk_ms901m_read_reg_by_id(emDevNum, ATK_MS901M_FRAME_ID_REG_ACCFSR, &g_atk_ms901m_fsr.accelerometer, 200);
    if (ret == 0)
    {
        return ATK_MS901M_ERROR;
    }
 ...
 * @param       timeout     : 获取数据最大等待超时时间，单位：毫秒（ms）
 ...
 * @param       accelerometer_dat: 加速度计数据结构体
 * @param       timeout          : 获取数据最大等待超时时间，单位：毫秒（ms）
 ...
 * @param       emDevNum: UART设备号
 * @param       port   : 指定端口
 * @param       pulse  : 端口PWM高电平的宽度，单位：纳秒（ns）
 * @param       timeout: 获取数据最大等待超时时间，单位：毫秒（ms）
...
uint8_t atk_ms901m_get_port_pwm_pulse(emUartDevNumTdf emDevNum, atk_ms901m_port_t port, uint16_t *pulse, uint32_t timeout)
{
    ...
    ret = atk_ms901m_read_reg_by_id(emDevNum, id, (uint8_t *)pulse, 100);
    if (ret == 0)
    {
        return ATK_MS901M_ERROR;
    }
...
uint8_t atk_ms901m_get_port_pwm_period(emUartDevNumTdf emDevNum, atk_ms901m_port_t port, uint16_t *period, uint32_t timeout)
{
    ...
    ret = atk_ms901m_read_reg_by_id(emDevNum, id, (uint8_t *)period, timeout);
    if (ret == 0)
    {
        return ATK_MS901M_ERROR;
    }
